...
#SBATCH -t 01:30:00
#SBATCH --mem=64GB
#SBATCH -o ./logs/weak_scaling_n4g4_%j.out
#SBATCH -e ./logs/weak_scaling_n4g4_%j.err
...
echo "=== 弱扩展性测试（每GPU 25K样本）: weak_scaling_n4g4 ==="
echo "节点数: $SLURM_NNODES"
echo "总GPU数: 4"
echo "作业ID: $SLURM_JOB_ID"
echo "GPU类型: 普通A100节点"
echo "CPU核心: 128 (4×32)"
echo "描述: 4GPU(100K)四节点"
echo "开始时间: $(date)"
...
srun python simple_main.py \
    ...
    --dataset-size 100000 \
    --benchmark-name "weak_scaling_n4g4" \
    --output-dir ./results
...
# 检查结果文件
echo "生成的结果文件:"
ls -la results/weak_scaling_n4g4_* 2>/dev/null || echo "未找到结果文件"
