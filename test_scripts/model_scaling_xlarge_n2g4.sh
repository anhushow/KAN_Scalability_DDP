#!/bin/bash
#SBATCH -J model_scaling_xlarge_n2g4
#SBATCH -N 2
...
#SBATCH -t 02:30:00
#SBATCH --mem=64GB
#SBATCH -o ./logs/model_scaling_xlarge_n2g4_%j.out
#SBATCH -e ./logs/model_scaling_xlarge_n2g4_%j.err
...
echo "=== 模型扩展性测试（固定50K样本）: model_scaling_xlarge_n2g4 ==="
echo "节点数: $SLURM_NNODES"
echo "总GPU数: 4"
echo "作业ID: $SLURM_JOB_ID"
echo "GPU类型: 普通A100节点"
echo "CPU核心: 128 (4×32)"
echo "描述: 超大型模型(~64K参数)4GPU"
echo "开始时间: $(date)"
...
srun python simple_main.py \
    ...
    --dataset-size 50000 \
    --benchmark-name "model_scaling_xlarge_n2g4" \
    --output-dir ./results
...
# 检查结果文件
echo "生成的结果文件:"
ls -la results/model_scaling_xlarge_n2g4_* 2>/dev/null || echo "未找到结果文件"
